...
# Latest snapshot of every fire still present in a source's newest fetch.
# The trailing WHERE is completed by each caller (whole source or one fire).
_CURRENT_FIRES_SQL = """
...
def _fires_frame(rows: list) -> gpd.GeoDataFrame:
    """Build the normalized EPSG:3857 fire frame from current-fire rows."""
...
def find_by_fire(conn: sqlite3.Connection, source: str,
                 term: str) -> Optional[gpd.GeoDataFrame]:
    """Current fires for a source whose displayed identifier equals term
    (exact, case-insensitive), queried in SQL rather than filtered in memory.

    Returns an empty frame when nothing matches, or None when the source has
    no data at all.
    """
    newest = latest_fetch(conn, source)
    if newest is None:
